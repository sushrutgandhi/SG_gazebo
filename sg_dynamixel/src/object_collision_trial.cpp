...
void pick(moveit::planning_interface::MoveGroup &group)
{
  ...
  g.pre_grasp_posture.joint_names.push_back("gripper_right_joint");
  ...
  g.grasp_posture.joint_names.push_back("gripper_right_joint");
...
void place(moveit::planning_interface::MoveGroup &group)
{
  ...
  g.post_place_posture.joint_names.push_back("gripper_right_joint");
...
{
  ros::init(argc, argv, "SGrobotpickplace");
  ros::AsyncSpinner spinner(5);
  spinner.start();

  ros::NodeHandle n;
  ros::Publisher pub_co = n.advertise<moveit_msgs::CollisionObject>("collision_object", 10);
  ros::Publisher pub_aco = n.advertise<moveit_msgs::AttachedCollisionObject>("attached_collision_object", 10);
